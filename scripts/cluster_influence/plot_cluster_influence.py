...
def parse_args():
    root = repo_root()
    parser = argparse.ArgumentParser(description="Plot a 2x2 cluster-influence subplot from LID CSV files.")
    parser.add_argument(
        "--csv-dir",
        default=str(root / "scripts" / "cluster_influence"),
        help="Directory containing the cluster-influence CSV files.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output PNG path. Defaults to results/plots/cluster_influence_subplot.png",
    )
...
def _plot_one_panel(ax, csv_path: Path) -> None:
    ...
    source_lang, target_lang = _extract_metadata(csv_path)
    ...
    for _, row in sae_df.iterrows():
        ax.scatter(row["ce_non_target_langs"], row["ce_target_token"], color="green", marker=MARKER_MAP.get(int(row["k"]), "o"), s=70, zorder=3)
    for _, row in learned_df.iterrows():
        ax.scatter(row["ce_non_target_langs"], row["ce_target_token"], color="orange", marker=MARKER_MAP.get(int(row["k"]), "o"), s=70, zorder=3)
    for _, row in sv_df.iterrows():
        ax.scatter(row["ce_non_target_langs"], row["ce_target_token"], color="blue", marker=MARKER_MAP.get(int(row["k"]), "o"), s=70, zorder=3)
    ax.scatter(no_sv_df["ce_non_target_langs"], no_sv_df["ce_target_token"], color="red", marker="D", s=90, zorder=3)

    target_name = LANG_CODE_TO_NAME.get(target_lang, target_lang)
    source_name = LANG_CODE_TO_NAME.get(source_lang, source_lang)
    ax.set_title(f"Target Language: {target_name}", fontsize=16, fontweight="bold")
...
def main():
    ...
    csv_paths = sorted(csv_dir.glob("*.csv"), key=lambda path: _extract_metadata(path)[1])
    if len(csv_paths) != 4:
    ...
    output = Path(args.output) if args.output else repo_root() / "results" / "plots" / "cluster_influence_subplot.png"
