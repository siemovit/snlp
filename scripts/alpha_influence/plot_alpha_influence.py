...
def parse_args():
    root = repo_root()
    parser = argparse.ArgumentParser(description="Plot a 2x2 alpha-influence subplot from LID CSV files.")
    parser.add_argument(
        "--csv-dir",
        default=str(root / "scripts" / "alpha_influence"),
        help="Directory containing the alpha sweep CSV files.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output PNG path. Defaults to results/plots/alpha_influence_subplot.png",
    )
    return parser.parse_args()


def _extract_metadata(path: Path) -> tuple[str, str, float]:
    match = re.search(r"_([a-z]+)_to_([a-z]+)_(?:layer\d+_)?alpha([0-9.]+)", path.name)
    if not match:
        raise ValueError(f"Could not parse source/target language and alpha from filename: {path.name}")
    return match.group(1), match.group(2), float(match.group(3))


def _plot_one_panel(ax, csv_path: Path) -> None:
    df = pd.read_csv(csv_path)
    source_lang, _target_lang, alpha = _extract_metadata(csv_path)
    ...
    ax.set_title(f"Original Language: {source_name} ($\\alpha={alpha:g}$)", fontsize=16, fontweight="bold")
...
def main():
    ...
    output = Path(args.output) if args.output else repo_root() / "results" / "plots" / "alpha_influence_subplot.png"
